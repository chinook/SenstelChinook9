...
//#ifndef WHEEL_RPM_KHZ_OUTPUT
        sensors.rpm_wheels *= 1000.0f/(float)TIME_ACQ * 60.0f / 48.0f;
//#endif
        wheel_rpm_counter = 0;
...
void WriteDataToCAN()
{
    ...
    float delta_wind = abs(sensors.wind_direction);
    ...
    */
    ...
    while (true)
    {
        ...
        if(flag_pc_out)
        {
            ...
            pc.printf("Pitch valid = %s\n\r", (pitch_valid) ? "TRUE" : "FALSE");
